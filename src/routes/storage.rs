...
use aws_sdk_s3::{config::{BehaviorVersion, Credentials, Region}, error::SdkError, Client};
use std::env;

use crate::{
    routes::structs::S3CheckResponse,
    state::AppState,
};
...
async fn s3_connection_check() -> impl axum::response::IntoResponse {
    ...
    match client.head_bucket().bucket(bucket_name.clone()).send().await {
        Ok(_) => (
            StatusCode::OK,
            Json(S3CheckResponse {
                status: "ok".to_string(),
